...
const getLocalDependencies = async (contractName, contractsBuildDirectory, contractsDirectory) => {
  ...
  return [
    ...
    ...flatten(await Promise.all(localBaseContracts
      .map(cont => getLocalDependencies(cont.contractName, contractsBuildDirectory, contractsDirectory)))),
  ];
};
